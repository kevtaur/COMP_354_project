...
class FunctionsTest {
			...
			if (scenario.exception == null) {
				try {
					assertEquals(new BigDecimal(scenario.expectedResult).setScale(5, RoundingMode.DOWN).doubleValue(),
							new BigDecimal(Functions.arccosine(scenario.input)).setScale(5, RoundingMode.DOWN)
									.doubleValue());
				} catch (Exception e) {
				...
				Throwable exception = assertThrows(scenario.exception.getClass(),
						() -> Functions.arccosine(scenario.input));
			}
		}
	}
	...
	/**
	 * @Test void testExponential() { fail("Not yet implemented"); }
	 */
...
, null),
	...
	/**
	 * @Test void testMeanAbsoluteDeviation() { fail("Not yet implemented"); }
	 */
	...
	void testLogDouble() {
		...
		Case cases[] = new Case[] { new Case("Input = 0.5", 0.5, -0.30102, null),
	...
	void testLogDoubleDouble() {
		...
		Case cases[] = new Case[] { new Case("Input = 0.5, Base = 2", 0.5, 2, -1.0, null),
		...
		for (Case scenario : cases) {
			...
			if (scenario.exception == null) {
				try {
					assertEquals(new BigDecimal(scenario.expectedResult).setScale(5, RoundingMode.DOWN).doubleValue(),
							new BigDecimal(Functions.log(scenario.input, scenario.base)).setScale(5, RoundingMode.DOWN)
									.doubleValue());
				} catch (Exception e) {
					fail("Unexpected exception thrown");
				}
				...
				Throwable exception = assertThrows(scenario.exception.getClass(),
						() -> Functions.log(scenario.input, scenario.base));
				assertEquals(scenario.exception.getMessage(), exception.getMessage());
			}
		}
	}
	...
	/**
	 * @Test void testSinh() { fail("Not yet implemented"); }
	 */
}
